    ...
    public class DictionaryPropertyDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (expanded)
            {
                ...
                {
                    keysProp.arraySize++;
                    var p = keysProp.GetArrayElementAtIndex(keysProp.arraySize - 1);
                    EditorHelper.SetPropertyValue(p, null);
                    valuesProp.arraySize = keysProp.arraySize;

                    EditorHelper.SetPropertyValue(valuesProp.GetArrayElementAtIndex(valuesProp.arraySize - 1), null);
                }
                if(GUI.Button(mRect, "-"))
                {
